...
uint16_t FLOW_BUF_CNT,FLOW_BUF_ONE_FRAME;
...
void OpticalFlow_Init()
{
  ...
  Set_Cutoff_Frequency(50, 6,&OpticalFlow_Gyro_Parameter);//同步相位
  ...
  //根据不同的光流配置，设置对应长度的数据缓冲区	
	if(Optical_Type_Present==1)//LC306
	{
		OpticalFlow_Is_Work=Config_Init_Uart();//光流传感器初始化
		FLOW_BUF_ONE_FRAME=14;		
	}
	...
	else if(Optical_Type_Present==3)//GL9306
	{
	  OpticalFlow_Is_Work=1;
	  FLOW_BUF_ONE_FRAME=9;
	}
	else if(Optical_Type_Present==4)//PMW3901
	{
	  OpticalFlow_Is_Work=1;
	  FLOW_BUF_ONE_FRAME=9;
	}
	
	FLOW_BUF_CNT=FLOW_BUF_ONE_FRAME*2;
}
...
{
  if(Optical_Type_Present==1)//优象LC306光流模块
  {
    for(uint16_t i=0;i<=FLOW_BUF_ONE_FRAME;i++)
    {
           ...
           &&OpticalFlow_Ringbuf.Ring_Buff[i+13]==0x55)//帧尾
      {
        ...
        opt_origin_data.qual=OpticalFlow_Ringbuf.Ring_Buff[i+10]; 
				opt_origin_data.version=OpticalFlow_Ringbuf.Ring_Buff[i+11]; 
	...
	else if(Optical_Type_Present==2)//优象LC302光流模块
  {
    for(uint16_t i=0;i<=FLOW_BUF_ONE_FRAME;i++)
    {
      if(OpticalFlow_Ringbuf.Ring_Buff[i]==0xfe//帧头
  ...
  else if(Optical_Type_Present==3)//无名创新GL9306光流模块
  {
    for(uint16_t i=0;i<=FLOW_BUF_ONE_FRAME;i++)
    {
      if(OpticalFlow_Ringbuf.Ring_Buff[i]==0xfe//帧头
  ...
  else if(Optical_Type_Present==4)//重楼科技光流模块PMW3901
  {
    for(uint16_t i=0;i<=FLOW_BUF_ONE_FRAME;i++)
    {
      if(OpticalFlow_Ringbuf.Ring_Buff[i]==0xfe//帧头
...
float OpticalFlow_Rotate_Complementary_Filter(float optflow_gyro,float gyro,uint8_t axis)
{
  ...
  {
    if(axis=='x') optflow_gyro_filter=optflow_gyro-constrain_float(gyro,-8.0f,8.0f);//4
    else optflow_gyro_filter=optflow_gyro-constrain_float(gyro,-8.0f,8.0f);
  }
	else if(Optical_Type_Present==3)
  {
	...
	else if(Optical_Type_Present==4)
  {
