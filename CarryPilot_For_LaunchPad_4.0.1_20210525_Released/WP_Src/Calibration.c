...
void Accel_Calibration_Check(void)
{
		...
		
  {
    ...
    cal_finished_flag=0;	
		Bling_Set(&Light_1,2000,1000,0.2,0,GPIO_PORTF_BASE,GPIO_PIN_1,1);
     ...
     &&Accel_Calibration_All_Finished==0)//6面全部校准完毕
  {
    ...
    if(cal_finished_flag==TRUE)//加速度计校准成功
    {
      ...
      Bling_Mode=0;//恢复正常指示模式
			Bling_Set(&Light_1,5000,2000,0.5,0,GPIO_PORTF_BASE,GPIO_PIN_1,0);
			NCLink_Send_Check_Flag[9]=0x01;//校准完毕后发送校准数据
    }
    else//加速度计校准失败
    {
      Bling_Mode=0;//恢复正常指示模式
			Bling_Set(&Light_1,5000,2000,0.1,0,GPIO_PORTF_BASE,GPIO_PIN_1,0);
...
void Mag_Calibration_Check(void)
{
    ...
    //进入磁力计校准模式
  {
    ...
    Mag_Calibration_Mode=3;
		Bling_Set(&Light_1,2000,2000,0.2,0,GPIO_PORTF_BASE,GPIO_PIN_1,1);		
...
uint8_t Mag_Calibartion(Vector3f *magdata,Vector3f_Body Circle_Angle_Calibartion)
{
       ...
       &&Mag_Is_Okay_Flag[2]==1)//三面数据全部采集完毕，计算磁力计零点
  {
    ...
    Mag_Offset[2]=(int16_t)(Mag_Offset_Read.z);
		...
		Bling_Set(&Light_1,4000,2000,0.5,0,GPIO_PORTF_BASE,GPIO_PIN_1,0);  
    //写入加速度零点偏执与磁力计中心偏执 
...
void RC_Calibration_Init()
{
  ...
  else//flash中不存在数据
  {
    for(i=0;i<8;i++)
    {
      ...
      RC_Data.cal[i].min=RC_BUTTOM_DEFAULT;
      RC_Data.cal[i].middle=RC_MIDDLE_DEFAULT;
      RC_Data.cal[i].deadband=RC_DEADBAND_DEFAULT;
    }
  }
}
/***********************************************************
...
{
  PWM_Output((uint16_t)(1.25*Receiver_PPM_Databuf[Thr_Chl_Num]),(uint16_t)(1.25*Receiver_PPM_Databuf[Thr_Chl_Num]),(uint16_t)(1.25*Receiver_PPM_Databuf[Thr_Chl_Num]),(uint16_t)(1.25*Receiver_PPM_Databuf[Thr_Chl_Num]),
             (uint16_t)(1.25*Receiver_PPM_Databuf[Thr_Chl_Num]),(uint16_t)(1.25*Receiver_PPM_Databuf[Thr_Chl_Num]),(uint16_t)(1.25*Receiver_PPM_Databuf[Thr_Chl_Num]),(uint16_t)(1.25*Receiver_PPM_Databuf[Thr_Chl_Num]));
}
...
void Horizontal_Calibration_Check(void)
{
             ...
             &&Controler_State==Lock_Controler)//必须为上锁状态才可以进入校准模式
  {
    ...
    //Hor_Accel_Offset[2]=(acce_sample_sum[2]/num_samples-AcceMax_1G)*ACCEL_TO_1G; //保存对应面的加速度计量
...
uint8_t Gyro_Calibration_Check(vector3f *gyro)
{
  ...
  static vector3f last_gyro;
   ...
   if(ABS(gyro->x-last_gyro.x)*scale<=5.0f
		   &&ABS(gyro->y-last_gyro.y)*scale<=5.0f
	      &&ABS(gyro->z-last_gyro.z)*scale<=5.0f
