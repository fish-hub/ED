...
{
	Bling_Set(&Light_2,500,50,0.2,0,GPIO_PORTF_BASE,GPIO_PIN_2,0);//红色  //用户移植时，重写此函数
}
...
void NCLink_SEND_StateMachine(void)
{
	...
	if(!NCLink_Send_Check_Status_Parameter())//判断地面站有无请求数据、是否需要发送应答
	{
		...
		{
			NCLink_Cnt=6;
			NCLink_Send_Obs_UOP(WP_Sensor.baro_altitude,GD_Distance,opt_gyro_data.y,opt_gyro_data.x);
		}
...
//														NamelessQuad.Speed[_PITCH]);
			NCLink_Send_Fusion_NE(kalman_k[0],
														kalman_k[1],
														0,
														0,
														0,
		...
		{
			NCLink_Cnt=9;
			NCLink_Send_Userdata(Total_Controller.High_Acce_Control.Integrate,
													 NamelessQuad.Speed[_YAW],
													 NamelessQuad.Acceleration[_YAW],
													 Origion_NamelessQuad.Acceleration[_YAW],
													 GD_Distance,NamelessQuad.Acce_Bias[_YAW]);
			//NCLink_Send_Userdata(opt_gyro_data.x,gyro_filter_data.x,tfdata.distance,tfdata.strength,us100.vel,GD_Distance);				
			//NCLink_Send_Userdata(opt_origin_data.pixel_flow_x_integral,opt_origin_data.pixel_flow_y_integral,opt_gyro_data.x,opt_gyro_data.y,gyro_filter_data.x,gyro_filter_data.y);
			//NCLink_Send_Userdata(OpticalFlow_Speed.y,OpticalFlow_SINS.Speed[_ROLL],OpticalFlow_Speed.x,OpticalFlow_SINS.Speed[_PITCH],OpticalFlow_SINS.Position[_ROLL],OpticalFlow_SINS.Position[_PITCH]);
		}
		else if(NCLink_Cnt==9)//传感器校准原始数据
		{
		  ...
		  NCLink_Send_CalRawdata1(Gyro_Safety_Calibration_Flag,
			                       ...
			                       WP_Sensor.accel_raw.x*ACCEL_TO_1G,
														 WP_Sensor.accel_raw.y*ACCEL_TO_1G,
														 WP_Sensor.accel_raw.z*ACCEL_TO_1G);
		}
